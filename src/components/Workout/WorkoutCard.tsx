...
const WorkoutCard = ({WorkoutItems}:WorkoutCardProps) => {
    return (
        <div className="bg-[#1A1D23] rounded-[10px] hover:border hover:border-[#C2F800]">
      <Image
      ...
      <div className="p-4">
        ...
        <div className="flex gap-3 border-dashed border-t pt-2 mt-2">
          <span className="flex items-center gap-1"><IoMdTime className="text-[#C2F800]"/>{WorkoutItems.duration}</span>
          <span className="flex items-center gap-1"> <FaFire className="text-[#C2F800]"/>{WorkoutItems.caloriesBurned}</span>
          <span className="flex items-center gap-1"><CiStar className="text-[#C2F800]"/>{WorkoutItems.rating}</span>
        </div>
      </div>
    </div>
    );
